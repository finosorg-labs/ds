...
linux:
	...
	@rm -f $(LINUX_BUILD_DIR)/libfinkit_ds_static_base.a
...
macos:
	...
	@echo "==> Cleaning intermediate build artifacts"
	@rm -f $(MACOS_BUILD_DIR)/libfinkit_ds_static_base.a
...
test: linux
	...
	@FC_BUILD_MODE=source CGO_CFLAGS_ALLOW="-mavx.*|-msse.*|-mfma|-mavx512.*" go test -vet=all -race -parallel=4 -v ./...
...
bench:
	...
	@if [ -f $(LINUX_BUILD_DIR)/benchmarks/ds_benchmarks ]; then \
		$(LINUX_BUILD_DIR)/benchmarks/ds_benchmarks; \
	else \
		echo "No C benchmarks found"; \
	fi
	@echo ""
	@echo "==> Running Go benchmarks"
	@FC_BUILD_MODE=source CGO_CFLAGS_ALLOW="-mavx.*|-msse.*|-mfma|-mavx512.*" go test -bench=. -benchmem ./...

format:
	@echo "==> Formatting C code with clang-format"
	@if command -v clang-format >/dev/null 2>&1; then \
		find ds-c include \( -name '*.c' -o -name '*.h' \) -exec clang-format -i {} \; ; \
	else \
...
clang-tidy:
	...
	@find ds-c include \( -name '*.c' -o -name '*.h' \) \
		! -name 'platform_win.c' \
...
cppcheck:
	...
	@find ds-c include \( -name '*.c' -o -name '*.h' \) -print | \
...
help:
	@echo "ds Makefile - Build and Test Targets"
	@echo ""
	@echo "Build Targets:"
